...
export class KidsService {
  ...
  async create(data: CreateKidDto): Promise<kid> {
    return this.prisma.kid.create({
      data: {
        fullname: data.fullname, // Asegúrate de que este valor esté presente
        age: data.age,           // Asegúrate de que este valor esté presente
        email: data.email,       // Asegúrate de que este valor esté presente
        score: data.score,       // Este campo puede ser opcional
      },
    });
  }
